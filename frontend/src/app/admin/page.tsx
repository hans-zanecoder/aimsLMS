...
type NavSection = 'dashboard' | 'students' | 'courses' | 'analytics' | 'permissions' | 'settings';
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (hash && ['students', 'courses', 'analytics', 'permissions', 'settings'].includes(hash)) {
  ...
  const fetchStudents = async () => {
    try {
      ...
      let students = studentsList; // Default to sample data
      
      try {
        const data = await studentsApi.getAll();
        console.log('Student data received:', data);
        
        // If we got valid data from the API, use it instead of sample data
        if (data && Array.isArray(data) && data.length > 0) {
          students = data;
        } else {
          console.log('No students found from API, using sample data');
        }
      } catch (apiError) {
      ...
      // Set the students data (either from API or sample data)
      setStudents(students);
      
      // Update recent users if we're on dashboard
      if (activeSection === 'dashboard') {
        updateRecentUsers(students);
      }
    } catch (error) {
      console.error('Error in fetchStudents function:', error);
      setStudentError('Failed to load students. Please try again.');
      
      // Fallback to sample data in case of any other errors
      setStudents(studentsList);
      
      if (activeSection === 'dashboard') {
        updateRecentUsers(studentsList);
      }
    } finally {
                ...
                <div className={styles.statusContent}>
                  <div className={styles.statusItem}>
                    ...
                    <div className={styles.statusBar}>
                      <div className={styles.statusProgress} style={{ width: '28%' }}></div>
                    </div>
                  </div>
                  <div className={styles.statusItem}>
                    <div className={styles.statusLabel}>
                    ...
                    <div className={styles.statusBar}>
                      <div className={`${styles.statusProgress} ${styles.warning}`} style={{ width: '64%' }}></div>
                    </div>
                  </div>
                  <div className={styles.statusGrid}>
                    <div className={styles.statusGridItem}>
                      <span className={styles.statusTitle}>Database Status</span>
                      <span className={`${styles.statusBadge} ${styles.success}`}>Healthy</span>
                    </div>
                    <div className={styles.statusGridItem}>
                      <span className={styles.statusTitle}>Last Backup</span>
                      <span className={styles.statusValue}>2 hours ago</span>
                    </div>
                    <div className={styles.statusGridItem}>
                      <span className={styles.statusTitle}>SSL Certificate</span>
                      <span className={`${styles.statusBadge} ${styles.success}`}>Valid</span>
                    </div>
                    <div className={styles.statusGridItem}>
                      <span className={styles.statusTitle}>Cache Status</span>
      ...
      case 'settings':
        return (
          <div className={styles.sectionContent}>
            ...
            <p className={styles.sectionDescription}>Configure system settings and preferences.</p>
